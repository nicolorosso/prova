...
#---------------------------------------------------------------------------------------------#
#ricerca_governo = st.sidebar.text('Spunta la casella per includere nella ricerca tutti i membri del governo')


#with st.sidebar:
    #with st.form(key='governo'):
       # governo_button = st.form_submit_button(label='Seleziona per ricerca governo')
...
def scraper(parole, users, since, until):
    ...
    with concurrent.futures.ProcessPoolExecutor(30) as executor:
        results = [executor.submit(scrape_tweets_for_user, parole, n, since, until) for p in parole for n in users]
    ...
    df.sort_values(by='Datetime', ascending=False, inplace=False)
    st.dataframe(df)
